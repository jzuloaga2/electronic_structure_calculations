      subroutine chartree(Lm,Nx,Nz,Wx,Wz,scr,var,b,in,accuracy)
      implicit none
      integer Lm,Nr,Nx,Nz
      double precision Wx,Wz,accuracy,scr
...
c     *********************************************************
      ...
      do ix = 1,Nx
         ...
         enddo
      enddo
c      write(*,*) 'chartree norm',norm
      if(norm.lt.0.0000000000000000001) then
        var = 0d0
        goto 111
      end if
c     ********************************************************
...
c     *********************************************************
      ...
      do ix = 1,Nx
         ...
         do iz = -Nz,Nz
            r(iz,ix)= -( 30d0*(gx+gz)  
     &     +(Lm*Lm-0.25d0)/(x*x)+1d0/(scr*scr))*in(iz,ix)
...
c        *************************************************
         do ix = 1,Nx
            ...
            do iz = -Nz,Nz
            q(iz,ix)= -( 30d0*(gx+gz)  
     &     +(Lm*Lm-0.25d0)/(x*x)+1d0/(scr*scr))*p(iz,ix)
...
c        *************************************************
         do ix = 1,Nx
            ...
            do iz = -Nz,Nz
            qp(iz,ix)= -( 30d0*(gx+gz)
     &     +(Lm*Lm-0.25d0)/(x*x)+1d0/(scr*scr))*pp(iz,ix)
...
c         write(*,*) it,check/Nr    
      enddo
      write(*,*) 'accuracy not achieved in green.f --chartree'
111   return
      end
